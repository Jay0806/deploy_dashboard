...
# expander is the option used to display the data or hide it
with st.expander("Full Data Table"):
    st.write(df)

# forms are used group or save multiple widgets to gether as per our requirements
...
with st.form("population-form"):
    ...
    with col1:
        ...
        start_quarter = st.selectbox("Quarter", options=["Q1", "Q2", "Q3", "Q4"], index=2, key="srart_q") # here key is used to differentiate the categories
        start_year = st.slider("Year", min_value =1991, max_value=2023, value=1991, step=1, key="start_y") # here in the value is the default one in this case it is min value will be default value

    with col2:
        st.write("choose end date")
        end_quarter = st.selectbox("Quarter", options=["Q1", "Q2", "Q3", "Q4"], index=0, key="end_q")
        end_year = st.slider("Year", min_value =1991, max_value=2023, value=2023, step=1, key="end_y")# here in the value is the default one in this case it is max value will be default value

    with col3:
        st.write("choose a location")
        target = st.selectbox("location", options=df.columns[1:], index=0) 
    submit_button = st.form_submit_button("Analyze", type="primary") # here analyze the lable and and its type is primary
...
def display_dashboard(start_date, end_date, target):
    ...
    print(start_date)
    print(end_date)
    print(df.loc[df['Quarter'] == start_date, target])
    print(df.loc[df['Quarter'] == end_date, target])
...
# validation check (python scalar error it will throw this error when it is not having any matching what we are looking)
